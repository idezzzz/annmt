...
//	one tree).
//----------------------------------------------------------------------
static int				IDX_TRIVIAL[] = {0};	// trivial point index // Tian: Thread-safe
ANNkd_leaf				*KD_TRIVIAL = NULL;		// trivial leaf node // Tian: Not thread-safe. But ensure by asking users to call annStart() first.
...
// This invisible namespace contains a global variable that allocates the tricky KD_TRIVIAL
// during construction to ensure a thread-safe behavior and frees the KD_TRIVIAL
// during deconstruction to avoid memory leak.
namespace
{

    // Should never create an object of this class
    // It has a global object, whenever the program ends
    // the descontructor of this object will automatically
    // call annClose();
    class ANNHelper
    {
    public:
		...
		ANNidxArray pi)					// point indices
{
	...
	{
		KD_TRIVIAL = new ANNkd_leaf(0, IDX_TRIVIAL);	// allocate it
		assert(false && "To ensure thread-safty, Function annStart() must be called before any call to ann search.");
	}
}
...
ANNkd_tree::ANNkd_tree(					// basic constructor
		...
		int dd,							// dimension
		int bs)							// bucket size
{  	ANNmaxPtsVisited = 0; SkeletonTree(n, dd, bs);  }			// construct skeleton tree
